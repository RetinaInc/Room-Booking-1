...
class Pager {
	...
	*/
	function Pager($tot_records=0, $limit=10, $page_var='page', $limit_var='limit') {
	...
	*/
	function printLimitCell() {
		$limits = $this->limits;
		$post_url = $_SERVER['PHP_SELF'] . '?' . preg_replace("/(\?|&|&amp;)$this->limit_var=[0-9]*/", '', $this->query_string);
		echo "<td align=\"right\">\n"
			. "<form name=\"limit_jump\" id=\"limit_jump\" style=\"margin: 0px;\">\n"
			. translate('Per page') . " <select name=\"$this->limit_var\" onchange=\"javascript: document.location.href='$post_url&amp;$this->limit_var=' + this.options[this.selectedIndex].value;\" class=\"$this->tb_class\" style=\"$this->tb_style\">\n";
