...
//				. ' WHERE resourceid IN (' . . ') AND ar.number_available = 0';
	...
	*/
	function del_res($id, $parentid, $del_recur, $date, $memberid, $update_timespan, $res_start_time) {
		$values = array($id);
		...
		if (!$del_recur && $update_timespan) {			// Delete some recurring reservations
			//$sql .= ' OR ru.resid = r.parentid OR r.parentid = ? AND r.start_date >= ?';
			$sql .= ' OR parentid = ? AND start_date >= ? AND start_date <=?';
			array_push($values, $parentid, $res_start_time, $update_timespan);
		}
	...
	*/
	function get_recur_ids($parentid, $start_date, $update_timespan, $res_start_time) {
		$return = array();

		// if update is for a limited time space 
		...
		if ($update_timespan) {
			...
			$result = $this->db->query($sql, array($parentid, $parentid, $res_start_time, $update_timespan));
