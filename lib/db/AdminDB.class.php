...
class AdminDB extends DBEngine {
	...
	*/
	function get_reservation_data($pager, $orders, $pending=null, $groupids = null, $paid = null) {
		$return = array();
		...
		$order_str = trim(preg_replace("/(res|l).$order(,)? (DESC|ASC)?(,)?/", '', 'res.start_date DESC, res.starttime, res.endtime, l.lname, l.fname, res.paid'));
		...
		$query = 'SELECT res.resid, res.start_date, res.end_date,
			...
			res.paid, res.amount, res.method, res.is_cancelled, res.total
		...
		if ($result->numRows() <= 0) {
			...
			{
				$this->err_msg = translate('Everyone one is paid up');
			}
			else {
	...
	* @return number of reservations pending approval
	*/
	function get_num_pending_pay() {
		$query = 'SELECT COUNT(*) as num FROM ' . $this->get_table(TBL_RESERVATIONS) . ' as reservations WHERE paid = false';
		$result = $this->db->getRow($query);
