...
class Admin {
	...
	*/
	function listUsers() {
		...
		print_user_table($pager, $users, $this->db->get_err());		// Print table of users
		$pager->printPages();
	}
	...
	* @param none
 	*/
 	function approvePayments() {
		...
		$res = $this->db->get_reservation_data($pager, $orders, false, false, true);		
		
		print_payment_pending($pager, $res, $this->db->get_err());		// Print table of users
 		$pager->printPages();									// Print pages
 	}
