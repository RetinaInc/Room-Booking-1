...
class User {
	...
	*/
	...
	/**
 	* Interface for seeing past reservations for user
 	* Provide a table to allow admin to approving/disapproving reservations
	* @param none
 	*/
 	function viewCustomRes($auth, $view) {
		
		// check if view is past or future
		switch ($view) {
			case 0:
				$dir = 0;
				$err = "This user has no previous reservations.";
				break;
			case 1:
				$dir = 1;
				$err = "This user has no furture reservations.";
				break;
		}
		
		
		$custom_pager = CmnFns::getNewPager();
		$custom_pager->setTextStyle('font-size: 10px;');
		$custom_pager->setTbClass('textbox');
		
	
		$orders = array('start_date', 'end_date', 'name', 'lname', 'starttime', 'endtime', 'paid');

		$res = $this->db->get_custom_reservations($auth, CmnFns::get_value_order($order), CmnFns::get_vert_order(), $dir);
		// $num = $this->db->get_num_pending_pay();	// Get number of records
		
		$num = count($res);		
		
		$custom_pager->setTotRecords($num);				// Pager method calls				
		$lim = $custom_pager->getLimit();
		$offset = $custom_pager->getOffset();
		
		print_custom_reservations($custom_pager, $res, $err, $dir, $offset, $lim);		// Print table of users
 		$custom_pager->printPages();									// Print pages
 	}	
