...
<?php
	...
	print_time_info($res->get_paid(), $res, $rs, !$res->is_blackout, (isset($rs['allow_multi']) && $rs['allow_multi'] == 1));	// Print time information
	...
	if($res->type == RES_TYPE_ADD && !$res->is_blackout)
	{
		print_money($res->get_paid(), $res->get_amount(), $res->get_method(), $res->type, $res, $res->total, $rs, $res->get_override());
	}
	else if (($res->user->userid == Auth::getCurrentID() || Auth::isAdmin()) &&  !$res->is_blackout)
	{
		print_money($res->get_paid(), $res->get_amount(), $res->get_method(), $res->type, $res, $res->total, $rs, $res->get_override());
	}
	...
	//	print_paid($res->get_paid(), $res->type);
	//	print_amount($res->get_paid(), $res->get_amount(), $res->type, $res);
	//	print_method($res->get_method(), $res->type);
	//  print_is_cancelled($res->get_is_cancelled(), $res-type);
	}
	...
	if (!empty($res->parentid) && ($res->type == RES_TYPE_MODIFY || $res->type == RES_TYPE_DELETE || $res->type == RES_TYPE_APPROVE)) {
		...
		print_update_dropdown($res->parentid);
	}

	if ($res->type == RES_TYPE_MODIFY) {
		print_del_checkbox($res);
	}
...
function print_buttons_and_hidden(&$res) {
					...
					global $conf;
					$groups = $res->user->get_groups();
					
					// loop through to pick out user groups
...
<?php
	...
	// loop through to see if user is a casual booker
	...
	$c_cur_time = time();
	$c_start_time = Time::getAdjustedDate($res->get_start_date(), $res->get_start());
	$months = $conf['app']['casual_booker'];
	$email = $conf['app']['adminEmail'];
	$adminphone = $conf['app']['adminPhone'];
	
	if ($casual && (($c_start_time - $c_cur_time) > (2678400 * $months)))
	{
		echo "<p align=\"left\"> You cannot book a reservation more than $months months in advance. <br /> Please call $adminphone
								or email $email <br /> to make any changes.<br />";
	}
...
* @global $conf
*/
function print_time_info($paid, $res, $rs, $print_min_max = true, $allow_multi = false) {
	global $conf;
...
<?php
        ...
        if (Auth::isAdmin() || (($type == RES_TYPE_ADD) ||  ($type == RES_TYPE_APPROVE)) || $res->is_pending || (($type == RES_TYPE_MODIFY) && !$paid && (($c_start_time - $c_cur_time) > (86400 * $days))  )) {
            ...
            for ($i = $startDay; $i < $endDay+$interval; $i += $interval) {
                ...
                if ( ($type==RES_TYPE_ADD && $res->get_end() + $rs['minres'] - $interval == $i  ) ) {
                    echo ' selected="selected" ';
				}
                else if ( $res->get_end()  == $i ) {
                    echo ' selected="selected" ';
				}				
                echo '>' . Time::formatTime($i) . "</option>\n";
            }
            ...
            echo '<td class="formNames" width="50%"><div id="div_start_date" style="float:left;width:86px;">' . Time::formatDate($start_date, '', false) . '</div>' . Time::formatTime($res->get_start()) . "</td><br \>"
			      . '<td class="formNames"><div id="div_end_date" style="float:left;width:86px;">' . Time::formatDate($end_date, '', false) . '</div>' . Time::formatTime($res->get_end()) . "</td><br \>";
...
/**
* update for the next X days, weeks, months
* @param string $parentid id of parent reservation
*/
function print_update_dropdown($parentid) {
	?>
	<p align="left"><input type="checkbox" name="update" value="<?php echo $parentid?>" /><?php echo 'Update Records for Next' ;?></p>
	&nbsp;&nbsp;<select name="length">
	<?php	
			for ($i=1;$i<=31;$i++) {
...
function print_del_checkbox($res) {
?> <?php
	global $conf;
	$days = $conf['app']['cancel'];
	$email = $conf['app']['adminEmail'];
	...
	if ($res->is_pending)
	{
		echo "<p align=\"left\"><input type=\"checkbox\" name=\"del\" value=\"true\" /> Delete?</p>";
	}
	else if ((!Auth::isAdmin()) && (($c_start_time - $c_cur_time) < (86400 * $days))) 
	{
		echo "<p align=\"left\"> You cannot cancel or amend a reservation with less than $days days' notice please call $adminphone
								or email $email to make any changes.";
	}
	else 
	{
		echo "<p align=\"left\"><input type=\"checkbox\" name=\"del\" value=\"true\" /> Delete?</p>";
...
// print out the amount box or amend it 

function print_money($paid, $amount, $method, $type, $res, $total, $rs, $override)
{ ?>
   <table width="100%" border="0" cellspacing="0" cellpadding="1">
    <tr class="tableBorder">
     <td>
      <table width="100%" border="0" cellspacing="1" cellpadding="0">
       <tr>
        <td colspan="2" class="cellColor"><h5 align="center"><?php echo 'Payment Information'?></h5></td></tr>

			    <tr>

					<?
					// lets collect the users group information
					global $conf;
					$groups = $res->user->get_groups();
					...
					if(Auth::isAdmin())
					{
						...
						?>	
						<tr>
						<td class="formNames"><? echo 'Paid?'?></td>
						<td class="cellColor" style="text-align: left;"> <?
								echo' <input type="radio" name="paid" value="1" > Yes ';
								echo' <input type="radio" name="paid" value="0" CHECKED>  No  ';
						?></td></tr>	<?	
							}
							else
							{
						...
						<td class="formNames"><? echo 'Paid?'?></td>
						<td class="cellColor" style="text-align: left;"> <?						
								echo' <input type="radio" name="paid" value="1" CHECKED> Yes ';
								echo' <input type="radio" name="paid" value="0">  No  ';
						?></td></tr>	<?		
							}
							// amount to pay
						...
						?>	
						<tr>
						<td class="formNames"><? echo 'Amount'?></td>
						<td class="cellColor" style="text-align: left;"> <?							
							echo 'Amount per session (75 minutes)'.' <br \> £<input type="text" name="amount" class="textbox" value="'. $amount.'"/>';
						?></td></tr>	<?
						...
						?>	
						<tr>
						<td class="formNames"><? echo 'Method of Payment'?></td>
						<td class="cellColor" style="text-align: left;"> <?							
							echo <<<_END
								Payment Method -
								<select name="method">
...
_END;
						?></td></tr><?
							...
							else 
							{
								...
								{
									case "Standing Order":
										$so = '<option value="Standing Order" selected>Standing Order</option>';
										$cheque = '<option value="Cheque">Cheque</option>';
										$bt='<option value="Bank Transfer" >Bank Transfer</option>';
										break;
									
									case "Cheque":
										$so = '<option value="Standing Order" >Standing Order</option>';
										$cheque = '<option value="Cheque" selected>Cheque</option>';
										$bt='<option value="Bank Transfer">Bank Transfer</option>';
										break;
									...
									case "Bank Transfer":
										$so = '<option value="Standing Order" >Standing Order</option>';
										$cheque = '<option value="Cheque" >Cheque</option>';
										$bt='	<option value="Bank Transfer"selected>Bank Transfer</option>';							
										break;
								}
						?>	
						<tr>
						<td class="formNames"><? echo 'Payment Method'?></td>
						<td class="cellColor" style="text-align: left;"> <?								
						...
						if (empty($amount))
						{
							?>	
							<tr>
							<td class="formNames"><? echo 'Total'?></td>
							<td class="cellColor" style="text-align: left;"> <?							
								echo 'Total will be calculated after creation';
							?></td></tr><?
						}
						else if ($type!=RES_TYPE_ADD && !$override)
						{
							$start = $res->get_start();
							$end = $res->get_end();
							(float)$no_ses = ($end - $start) / $rs['minres'];
							$total = ceil($no_ses) * $amount;
							?>	
							<tr>
							<td class="formNames"><? echo 'Total'?></td>
							<td class="cellColor" style="text-align: left;"> <?							
								echo '£<input type="text"  name="total" class="textbox" value="'. $total.'"/>';
							?></td></tr><?							
						}
						else if ($type!=RES_TYPE_ADD && $override)
						{
						...
						?>	
						<tr>
						<td class="formNames"><? echo 'Override Total?'?></td>
						<td class="cellColor" style="text-align: left;"> <?
								echo' <input type="radio" name="override" value="1" > Yes ';
								echo' <input type="radio" name="override" value="0" CHECKED>  No  ';
						?></td></tr>	<?	
							}
							else
							{
						...
						<td class="formNames"><? echo 'Override Total?'?></td>
						<td class="cellColor" style="text-align: left;"> <?						
								echo' <input type="radio" name="override" value="1" CHECKED> Yes ';
								echo' <input type="radio" name="override" value="0">  No  ';
						?></td></tr>	<?		
							}					
					...
					// begin non admin method payment
					else 
					{
						if (($type == RES_TYPE_ADD || $type == RES_TYPE_MODIFY || $type == RES_TYPE_APPROVE) && (!$paid))
						{
					?>	
					<tr>
					<td class="formNames"><? echo 'Paid?'?></td>
					<td class="cellColor" style="text-align: left;"> <?							
							echo' <input type="radio" name="paid" value="1" DISABLED> Yes ';
							echo' <input type="radio" name="paid" value="0" CHECKED DISABLED>  No  ';
					...
					<td class="formNames"><? echo 'Paid?'?></td>
					<td class="cellColor" style="text-align: left;"> <?							
							echo' <input type="radio" name="paid" value="1" CHECKED DISABLED> Yes ';
							echo' <input type="radio" name="paid" value="0" DISABLED>  No  ';
					...
					<td class="formNames"><? echo 'Paid?'?></td>
					<td class="cellColor" style="text-align: left;"> <?						
							echo "Your not a member of any groups so cant make bookings <br \> please contact admin";
					?></td></tr><?	
						}
						...
						{	
							...
							// check to see if amount is empty
								if (empty($amount))
								{
									...
									for ($i = 0; ; ++$i)
									{
										switch ($members[$i]) {
											case "External":
												$amount = $conf['app']['group_rates']["External"];
												break;
											case "Internal":
												$amount = $conf['app']['group_rates']['Internal'];
												break;
												
											case "Reduced Rate":
												$amount = $conf['app']['group_rates']['Reduced Rate'];
												break;
											case "No Charge":
												$amount = $conf['app']['group_rates']['Reduced Rate'];
												break;
									...
									<td class="formNames"><? echo 'Amount'?></td>
									<td class="cellColor" style="text-align: left;"> <?								
									echo 'Amount per session (75 minutes) '.'<input type="text" '.$disabled.' name="amount" class="textbox" value="£'. $amount.'"/>';	
							...
							// print summary of payment methods
							if (empty($method))
							{
						?>	
						<tr>
...
_END;
						?></td></tr><?
							...
							<td class="formNames"><? echo 'Payment Method'?></td>
							<td class="cellColor" style="text-align: left;"> <?							
								echo '<select name="method" disabled>'
										.$so.$cheque.$bt.'</select>';
							?></td></tr><?
							
							//echo 'Payment Method'.'<input type="text" '.$disabled.' name="amount" class="textbox" value="'. $method.'"/>';
							}
						// print total 
						if (empty($total) && $type==RES_TYPE_ADD)
						{
							?>	
							<tr>
							<td class="formNames"><? echo 'Total'?></td>
						...
						else if ($type!=RES_TYPE_ADD && !$override)
						{
							...
							<td class="formNames"><? echo 'Total'?></td>
							<td class="cellColor" style="text-align: left;"> <?							
								echo '<input type="text" disabled name="total" class="textbox" value="£'. $total.'"/>';
...
function print_amount($paid, $amount, $type, $res)
{ echo "<br \>"; 
if($paid)
{
	$title = 'Amount Paid';
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="1"> 
		<tr class="tableBorder">
			<td>
			  <table width="100%" border="0" cellspacing="1" cellpadding="0">
			    <tr>
					<td class="cellColor"><h5 align="center"><? echo "$title"?></td>
					<td class="cellColor" style="text-align: left;"> 
					<?
					global $conf;
					$res_id = $res->user->userid;
					$cur_user_id = (Auth::getCurrentID());
					...
					if (($type == RES_TYPE_ADD || $type == RES_TYPE_MODIFY || $type == RES_TYPE_APPROVE)) 
					{
						...
						{
							echo "Your a member of clashing groups, please contact admin";
						}
						else	{
								if (Auth::isAdmin())
								{
									$disabled = '';
									echo 'Amound per session (75 minutes) £'.'<input type="text" '.$disabled.' name="amount" class="textbox" value="'. $amount.'"/>';
								}
								else 
								{
									$disabled = 'disabled';
						...
						else if ($type == RES_TYPE_VIEW)
						{
							if ($cur_user_id == $res_id)
							{
								echo 'Amound per session (75 minutes) £'.'<input type="text" disabled name="amount" class="textbox" value="'. $amount.'"/>';
							}
							else if (Auth::isAdmin())
							{
								echo 'Amound per session (75 minutes) £'.'<input type="text" name="amount" class="textbox" value="'. $amount.'"/>';
							}
						}
