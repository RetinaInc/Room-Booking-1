...
<table width="100%" border="0" cellspacing="0" cellpadding="1" align="center">
  <tr>
    <td class="tableBorder">
      ...
      <table width="100%" border="0" cellspacing="1" cellpadding="0">
        <tr class="rowHeaders">
		  ...
		  <td width="8%"><?php echo translate('Paid')?></td>
		  <td width="8%"><?php echo translate('Total')?></td>		  
	...
	for ($i = 0; is_array($res) && $i < count($res); $i++) {
		...
		$class = 'cellColor' . ($i%2);
		$c_cur_time = time();
		$c_start_time = Time::getAdjustedDate($rs['start_date'], $rs['starttime']);
		$paid = $rs['paid'];
		((!$paid) ? $msg = "No" : $msg = "Yes");
		...
		if (Auth::isAdmin() || ($res[$i]['is_pending']) || (($c_start_time - $c_cur_time) > (86400 * $days))) 
		{
			echo "        <tr class=\"$class\" align=\"center\" style=\"background-color:$bgcolor;\">"
	                    ...
	                    . '			 <td>' . $link->getLink("javascript: reserve('m','','','" . $rs['resid']. "');", $msg, '', '', translate('View this reservation information')) . '</td>'		
						. '          <td>£' . $rs['total'] . '</td>'						
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr style="padding: 5px;" class="cellColor">
          <td colspan="2">
            ...
            <p><b>&raquo;</b>
              <?php $link->doLink('past_reservations.php', translate('View Past Bookings')) ?>
            </p>			
			<p><b>&raquo;</b>
			...
			if ($is_admin) {
				echo
				 '<p><b>&raquo;</b> ' .  $link->getLink('pending_reservations.php', translate('Approve Pending Reservations')) . "</p>\n"
