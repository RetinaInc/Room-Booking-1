...
/**
* Prints out the user payment table
* @param Object $pager pager object
* @param mixed $users array of user data
* @param string $err last database error
*/
function print_user_table(&$pager, $users, $err) {
	global $link;
	global $conf;
	$util = new Utility();
...
<table width="100%" border="0" cellspacing="0" cellpadding="1" align="center">
  <tr>
	...
	for ($i = 0; is_array($users) && $i < count($users); $i++) {
		...
		echo "<tr class=\"cellColor" . ($i%2) . "\" align=\"center\" id=\"tr$i\">\n"
             . '<td style="text-align:left">' . $link->getLink("javascript: viewUserAdmin('" . $cur['memberid'] . "');", $fname . ' ' . $lname, '', '', translate('View information about', array($fname,$lname))) . '</td>'
...
* @param string $err last database error
*/
function print_custom_reservations(&$pager, $res, $err, $dir, $offset, $lim) {
	global $link;
	global $conf;
	$util = new Utility();

	if ($dir) {
		$message = translate('Future Reservations');
		$error = 'This user has no future reservations.';
	}
	else {
		$message = translate('Past Reservations');
		$error = 'This user has no past reservations.';
	}
?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="1" align="center">
  <tr>
    <td class="tableBorder">
      <table width="100%" border="0" cellspacing="1" cellpadding="0">
        <tr>
          <td colspan="9" class="tableTitle">&#8250; <?php echo $message?></td>
        </tr>
		<?php echo"
        <tr class=\"rowHeaders\">
          <td width=\"9%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'start_date'), translate('Start Date')) . "</td>
		  <td width=\"9%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'end_date'), translate('End Date')) . "</td>
          <td width=\"10%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'name'), translate('Resource')) . "</td>
          <td width=\"10%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'starttime'), translate('Start Time')) . "</td>
          <td width=\"10%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'endtime'), translate('End Time')) . "</td>
    <td width=\"18%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'created'), translate('Created')) . "</td>		  
          <td width=\"8%%\">" . translate('Paid') . "</td>
		  <td width=\"8%\">" . translate('Total') . "</td>
          <td width=\"7.5%\">" . translate('View') . "</td>
          <td width=\"10.5%\">" . translate('Delete') . "</td>		  
        </tr>";
		
	// check for future or past and set highlighting
	$dir ? $color = 1 : $color = 0;

	// Write message if they have no reservations
	if (!$res)
		echo '<tr class="cellColor"><td colspan="10" align="center">' . $error . '</td></tr>';

	// For each reservation, clean up the date/time and print it
	for ($i = $offset; is_array($res) && ($i < $offset+$lim && $i < $pager->tot_records); $i++) {
		$class = 'cellColor' . ($i%2);
		$cur = $res[$i];
		$fname = $cur['fname'];
		$lname = $cur['lname'];
		...
		if (!$paid) {
			$class = 'cpanelCell';	
			$bgcolor = '#' . $conf['ui']['cancelled'][0]['color'];
		}
		
		if (Auth::isAdmin() || ($res[$i]['is_pending']) || (($c_start_time - $c_cur_time) > (86400 * $days))) {
			$delete = '<td>' . $link->getLink("javascript: reserve('d','','','" . $cur['resid']. "');", translate('Delete'), '', '', translate('Delete this reservation')) . '</td>';
		}
		else {
			$delete = '';
		}
        ...
        echo "<tr class=\"$class\" align=\"center\" style=\"background-color:$bgcolor;\">"
					...
					. '<td>' . Time::formatTime($cur['starttime']) . '</td>'
					. '<td>' . Time::formatTime($cur['endtime']) . '</td>'
					. '<td>' . Time::formatDateTime($cur['created']) . '</td>'					
                    . '<td>' . $link->getLink("javascript: reserve('m','','','" . $cur['resid']. "');", $msg, '', '', translate('View this reservation information')) . '</td>'
					. '<td>£' . $cur['total'] . '</td>'	
					. '<td>' . $link->getLink("javascript: reserve('m','','','" . $cur['resid'] . "');", translate('View'), '', '', translate('View this reservation')) . '</td>'					
					. $delete
					. "</tr>";
	}
    ?>
...
/**
* Interface for viewing past reservations
* Provide a table to allow admin to approve or delete reservations
* @param Object $pager pager object
* @param mixed $res reservation data
* @param string $err last database error
...
function print_payment_pending($pager, $res, $err) {
	global $link;
	$util = new Utility();
...
<table width="100%" border="0" cellspacing="0" cellpadding="1" align="center">
  <tr>
    <td class="tableBorder">
      <table width="100%" border="0" cellspacing="1" cellpadding="0">
        <tr>
          <td colspan="9" class="tableTitle">&#8250; <?php echo translate('Pending Reservation Payment')?></td>
        </tr>
		<?php echo"
        <tr class=\"rowHeaders\">
          <td width=\"10%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'start_date'), translate('Start Date')) . "</td>
		  <td width=\"10%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'end_date'), translate('End Date')) . "</td>
          <td width=\"16%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'lname'), translate('User')) . "</td>
          <td width=\"15%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'name'), translate('Resource')) . "</td>
          <td width=\"10%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'starttime'), translate('Start Time')) . "</td>
          <td width=\"10%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'endtime'), translate('End Time')) . "</td>
          <td width=\"10.5%\">" . translate('Paid') . "</td>
		  <td width=\"8%\">" . translate('Total') . "</td>
          <td width=\"10.5%\">" . translate('Delete') . "</td>
        </tr>";
  ...
  <tr>
    <td class="tableBorder">
      <table width="100%" border="0" cellspacing="1" cellpadding="0">
        ...
        <tr class=\"rowHeaders\">
		  ...
		  <td width=\"10%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'end_date'), translate('End Date')) . "</td>
          <td width=\"20%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'lname'), translate('User')) . "</td>
          <td width=\"14%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'name'), translate('Resource Name')) . "</td>
          <td width=\"9%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'starttime'), translate('Start Time')) . "</td>
          <td width=\"9%\">" . $link->getLink($_SERVER['PHP_SELF'] . $util->getSortingUrl($_SERVER['QUERY_STRING'], 'endtime'), translate('End Time')) . "</td>
		  <td width=\"7%\">" . translate('Paid') . "</td>
          <td width=\"7%\">" . translate('View') . "</td>
	...
	for ($i = 0; is_array($res) && $i < count($res); $i++) {
        ...
        echo "<tr class=\"$class\" align=\"center\" style=\"background-color:$bgcolor;\">"
					...
					. '<td>' . Time::formatTime($cur['starttime']) . '</td>'
					. '<td>' . Time::formatTime($cur['endtime']) . '</td>'
					. '<td>' . $link->getLink("javascript: reserve('m','','','" . $cur['resid']. "');", $msg, '', '', translate('Paid')) . '</td>'
